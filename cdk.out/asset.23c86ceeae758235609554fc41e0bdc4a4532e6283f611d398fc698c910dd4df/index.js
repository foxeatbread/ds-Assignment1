...
var isValidQueryParams = ajv.compile(
  types_schema_default.definitions["MovieCastMemberQueryParams"] || {}
);
var handler = async (event, context) => {
  try {
    ...
    if (!isValidQueryParams(queryParams)) {
      return {
        ...
        body: JSON.stringify({
          ...
          schema: types_schema_default.definitions["MovieCastMemberQueryParams"]
        })
      };
    }
    if (!movieId) {
    ...
    const commandOutput = await ddbDocClient.send(
      new import_client_dynamodb.QueryCommand(commandInput)
    );
